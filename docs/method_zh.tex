...
\vspace{2pt}
{\small 图~\thefigure：方法概览。两个耦合机制将交互历史转化为可追溯的风险抽象，并按决策职责分配证据；确定性模块保留执行权限。}
\end{center}
...
本文提出一种风险经验驱动的多角色框架，其核心创新是将\textbf{可追溯风险抽象}与\textbf{面向职责的证据分配}相互耦合。给定交互历史 $\mathcal{H}_t$，第一项机制将完整交互转化为紧凑的单案例卡片和跨案例规则，同时保留支撑每层抽象的证据。给定当前规划输入 $\mathcal{X}_t=(g_t,r_t,e_t,\mathcal{A}_t)$，第二项机制针对不同决策职责选择不同的证据包。因此，长期经验首先按风险结构压缩，再根据其在协商中的用途选择性呈现。

Task Advocate、Risk Critic 和 Safety Decision Agent 分别解释所分配的证据，并输出包含证据 ID 的结构化建议。确定性约束优化器随后从允许动作集 $\mathcal{A}_t$ 中选择语义动作，独立 Safety Guard 在执行前对其进行验证。因此，Agent 提供由证据支撑的语义判断，但不拥有可执行控制权限。第 3.2 节定义可追溯抽象机制，第 3.3 节定义面向职责的证据分配及其决策集成。

\subsection{可追溯的分层风险记忆}

第一项机制构造了具有明确决策功能的分层记忆：L0 经历集 $\mathcal{E}_t$ 保留完整交互，L1 卡片集 $\mathcal{M}_t$ 显式提取单个案例的风险结构，L2 规则集 $\mathcal{Q}_t$ 编码由多个案例共同支持的模式。其关键性质是可追溯抽象，即每次压缩均通过稳定 ID 保留与输入证据的关联。这些关联用于计算规则支持度、校验决策引用，并在新证据与规则冲突时定位受影响的案例。因此，该层次结构既压缩决策上下文，又使每层抽象能够由经验验证和更新。
...
第二项机制以决策职责为条件分配证据。在相同状态和记忆下，Advocate 优先获取任务推进与成功缓解证据，Critic 优先获取失败案例与停止条件，Decision Agent 优先获取用于权衡任务收益和风险的证据。其关键不只是设置多个角色，而是将角色条件置于检索过程内部，使不同职责在协商前即获得不同证据。给定 $\mathcal{X}_t$，每张卡片产生特征向量 $\phi_i=[\mathrm{sim},\mathrm{sev},\mathrm{rel},\mathrm{rec},\mathrm{fit}]\in[0,1]^5$，分别表示情境相似度、严重度、可靠性、时效性和角色适配度。角色权重 $w^{(r)}$ 将这些公共特征转化为面向职责的相关性。
...
令 $\mathcal{D}_t$ 表示三个角色的报告，$\mathcal{F}_t$ 表示满足硬约束和 L2 禁止规则的动作集合。对于动作 $a$，$U(a)$ 是任务效用，$R(a\mid\mathcal{B}_{\mathrm{dec}}^*)$ 是场景与记忆的联合风险，$Z(a\mid\mathcal{D}_t,\mathcal{B}_{\mathrm{dec}}^*)$ 综合观测置信度、记忆覆盖和角色分歧。

阈值 $\rho$ 和 $\eta$ 分别约束风险和不确定性。
...
\end{equation}
规则冲突、输出无效、可行集为空或 Safety Guard 拒绝动作时，框架触发 \code{safe\_stop}、\code{observe\_again} 或人工复核。该确定性边界为上述两项记忆机制提供执行保障，防止语义建议绕过动作约束。
