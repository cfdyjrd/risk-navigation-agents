...
本文通过逐层抽象构造风险证据。L0 经历集 $\mathcal{E}_t$ 保留完整交互，L1 卡片集 $\mathcal{M}_t$ 提取单个案例的风险结构，L2 规则集 $\mathcal{Q}_t$ 归纳由多个案例共同支持的风险模式。稳定 ID 使每条规则能够经由来源卡片追溯至原始记录。因此，该层次结构既减少决策时需要加载的历史上下文，也保留检查和修订抽象结果所需的证据。
...
\label{eq:rule-zh}
\end{equation}
这里 $c_j$ 衡量明确经验支持的程度，而非风险发生概率。当来源数、跨场景一致性和可靠性达到预设阈值时，规则转为 \textit{active}；矛盾或过期证据将修订其触发范围或使其转为 \textit{retired}。版本化来源 ID 在更新中始终保留 $Q_j\rightarrow M_i\rightarrow E_i$ 的证据链。
...
\end{equation}
对于 $(\mathrm{sim},\mathrm{sev},\mathrm{rel},\mathrm{rec},\mathrm{fit})$，本文取 $w^{(\mathrm{adv})}=(.35,.05,.20,.05,.35)$、$w^{(\mathrm{crit})}=(.30,.25,.15,.05,.25)$ 和 $w^{(\mathrm{dec})}=(.35,.20,.20,.10,.15)$。算法~\ref{alg:retrieval} 以贪心边际增益近似求解式~\eqref{eq:retrieval-zh}，实验部分评估分角色权重的作用。

每个证据包记录角色与检索版本、选中的 L1 卡片和 L2 规则、得分、匹配原因、预算用量及来源 ID。Agent 通过 \code{cited\_experience\_ids} 和 \code{cited\_rule\_ids} 声明所用证据；框架拒绝证据包外的 ID，从而保留完整的检索至决策审计链。
