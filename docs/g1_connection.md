...
顶层时间戳仍用最旧必需数据的有效时间。DDS 写入端与接收端时钟不同步时，
采集器保留原始写入时间，且只有在滑动窗口证明源时间持续递增、与本机单调时钟
速率一致、接收间隔正常且钟差稳定后，才使用本机接收时间作新鲜度判定。
冻结、倒退、非实时突发回放/积压或钟差跳变都会使该证据失效；按原速伪造且时间
连续的回放仍需由数据源身份认证和现场隔离防范。状态类型仅是平台一致性证据，
不是设备身份认证。
...
采集时长默认 10 秒、最多 1200 秒，父进程总超时为采集时长加 15 秒。延长上限只让
同一只读订阅和 GET_FSM 查询在 S1 试次期间保持运行，不修改 DDS 域、发布器、系统
时钟、FSM 或运动状态。
采集结束后源时间戳不再更新，缓存自然过期；这不是后台常驻服务。
`state_cache_ready` 仅检查四种状态的时间有效性、DDS 流活性证据和采集错误，
`motion_ready` 始终为 false。缺失、未来、超过一秒，或源时间冻结/倒退、时钟速率
不一致、接收断流、钟差不稳定，都会使缓存未就绪并返回退出码 1；0 也不表示可以运动。

首次实测采集到了电量 **81%** 和 FSM **1**（当时的状态，不是持续保证）。
现场 SDK 的 `Damp()` 写入状态 1；本次未进行任何模式切换。
DDS 写入端时间戳比本机接收时间落后约 **24.7 秒**。主机 `timedatectl`
显示 NTP 同步，但这不能证明 DDS 发布端时钟一致，也不能仅凭此区分时钟偏差和延迟。
需要核查发布端时间与链路。采集器明确分别报告 `source_age_s`、`receipt_age_s`、
实际采用的判定依据和滑动窗口证据；不会改写原始 DDS 时间或给它直接加固定偏移。
...
3. 由现场操作员核定允许的运控状态、姿态限制、独立停止和目标到达判据。
...
不同参与者不证明它们位于不同电脑，因此不能把 GET 响应的校时区间自动应用到
全部状态流。内部邻居 `192.168.123.161` 的 SSH 22 端口明确拒绝连接，未取得其
系统时钟配置。

同日再次实测时，三个遥测流各自的 64 样本窗口覆盖约 3.53 秒，源时钟/本机单调
时钟速率比分别约 0.9999、0.9998、1.0074，最大接收间隔约 58 毫秒，窗口内钟差
波动分别约 1.8、4.3、48.5 毫秒。三个流均通过独立活性检查；约 24.85 秒的原始
钟差仍完整保留并报告，状态缓存可据本机接收时间判定为新鲜。当前实测 FSM 为
500，电量读数曾从 38% 降至 31%，均只是当时读数。

这项改动仅发生在本项目订阅端，没有修改 DDS 域、发布器、机器人或发布端系统时钟，
也没有增加运动/模式 API。现场宽度、雷达标定、独立急停与目标到达等缺项仍保留。

当前前向相机只读快照显示机器人位于有人和办公家具的室内区域；相机不覆盖脚边、
侧后方或台阶。`pre_safe_clouds`、`safe_clouds`、`collision_clouds`、
`warning_clouds` 虽能发现 DDS 发布端，但各自 3 秒内均未收到数据。因此本轮没有发送
非零速度或零速度控制请求，不能据此宣称当前场地允许运动。

## 后续受限直行验证状态

在操作者另行确认前方清空和急停就绪后，后续受限直行诊断证明 G1 速度命令链路能产生
可测前向位移；一次 1 m 目标测试在约 0.148 m 处因实测速度/偏航包络检查失效关闭，故该次
不能记作“完成 1 m”。操作者随后报告机器人已能正常运动，但这也不等于 S1 通道策略已
验证。上述记录不得转写成窄通道历史成功卡或碰撞失败卡；S1 仍须按
[`S1_narrow_corridor_operation.md`](S1_narrow_corridor_operation.md) 完成动态 W、点云/TF、
纸箱几何和 A/B 全通道低速预实验。诊断脚本本身不修改 FSM、姿态、DDS 配置或系统时钟。
